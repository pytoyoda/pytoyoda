...
def login():
    ...
    if request.method == "POST":
        ...
        try:
            ...
            session["password"] = (
                password  # Storing password in session is not recommended for production
            )
...
def dashboard():
    ...
    try:
        ...
        if "Authentication failed" in str(e) or "TOKEN_INVALID" in str(
            e
        ):  # Crude check
    ...
    return render_template(
        "dashboard.html", username=username, vehicles=vehicles, error=error
    )
...
def vehicle_details(vin):
    ...
    try:
        ...
        if found_vehicle_obj:
            run_async(
                found_vehicle_obj.update()
            )  # Refresh data for the specific vehicle

            vehicle_stats["dashboard_info"] = (
                found_vehicle_obj.dashboard.model_dump()
                if found_vehicle_obj.dashboard
                else None
            )
            vehicle_stats["electric_status"] = (
                found_vehicle_obj.electric_status.model_dump()
                if found_vehicle_obj.electric_status
                else None
            )
            vehicle_stats["location_info"] = (
                found_vehicle_obj.location.model_dump()
                if found_vehicle_obj.location
                else None
            )
            vehicle_stats["lock_status"] = (
                found_vehicle_obj.lock_status.model_dump()
                if found_vehicle_obj.lock_status
                else None
            )
            vehicle_stats["notifications"] = (
                [n.model_dump() for n in found_vehicle_obj.notifications]
                if found_vehicle_obj.notifications
                else None
            )
            ...
            vehicle_stats["latest_service"] = (
                latest_service_obj.model_dump() if latest_service_obj else None
            )

            vehicle_stats["last_trip"] = (
                found_vehicle_obj.last_trip.model_dump()
                if found_vehicle_obj.last_trip
                else None
            )

            # Daily summary needs an async call
            daily_summary_obj = run_async(found_vehicle_obj.get_current_day_summary())
            vehicle_stats["daily_summary"] = (
                daily_summary_obj.model_dump() if daily_summary_obj else None
            )
    ...
    return render_template(
        ...
        error=error,
        username=username,
    )  # Pass username for consistency if needed by layout
